...
class C45:
    ...
    def doC45(self, box, atribList, atribDiv, atribVal):

        # ---------------- Calcular el gain ratio mas alto ----------------
        gr = {}
        numelems = box.getNumElems()
        # Calcular el gain ratio de todos los atributos
        for col in atribList:
            ...
            gr[info.gainRatio(box.getEntropy(), numelems, A)] = col

        # ---------------- Dividir con el mejor atributo ----------------
        choseAtrib = gr[max(gr)]
        ...
        for val in range(self.colDomain[choseAtrib - 1] + 1):
            ...
            if (newBox.getEntropy() != 0 and newAtribList != []):
                self.doC45(newBox, newAtribList, newAtribDiv, atribVal + [val])
